...
class ReactUeditor extends React.Component {
  ...
  componentDidMount() {
    ...
    container.UE_LOADING_PROMISE.then(() => {
      this.tempfileInput = container.document.getElementById(this.fileInputID);
      this.initEditor();
    });
  }
  ...
  createScript = url => {
    ...
    let scriptTags = container.document.querySelectorAll('script');
    let len = scriptTags.length;
    let i = 0;
    let _url = container.location.origin + url;
    return new Promise((resolve, reject) => {
      ...
      let node = container.document.createElement('script');
      node.src = url;
      node.onload = resolve;
  ...
  initEditor = () => {
    const { config, plugins, onChange, value, getRef, onReady, container } = this.props;
    ...
    this.ueditor = config
      ? container.UE.getEditor(this.containerID, config)
      : container.UE.getEditor(this.containerID);
    ...
    this.ueditor.ready(() => {
      ...
      domUtils.on(this.ueditor.body, 'cut', () => {
        setTimeout(() => {
